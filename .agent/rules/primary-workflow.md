...
- Before you start, delegate to `.agent/agents/planner.md` agent to create a implementation plan with TODO tasks in `./plans` directory.
...
- Delegate to `.agent/agents/docs-manager.md` agent to update docs in `./docs` directory if any.

#### 5. Debugging

- When a user report bugs or issues on the server or a CI/CD pipeline, delegate to `debugger` agent to run tests and analyze the summary report.
- Read the summary report from `debugger` agent and implement the fix.
- Delegate to `.agent/agents/tester.md` agent to run tests and analyze the summary report.
- If the `.agent/agents/tester.md` agent reports failed tests, fix them follow the recommendations and repeat from the **Step 2**.
